...
<html>
	...
	<body>
		...
		<div id="content">
			...
			<form action="selectDepartment.action" method="post" name="searchForm">
			<input type="hidden" name="pagenum" id="pagenum"/>
			<div class="container-fluid">
				<div class="row-fluid">
					<div class="control-group">
						<span class="span3">
							名称：<input id="ename" name="departmentName" type="text" style="width:65%" value=""/>
						</span>
						<span class="span3">
							状态：<select id="eflag" name="departmentMark" style="width:70%">
			...
			<div class="row-fluid">
				<div class="span12" style="padding-left:30px;">
					...
					<script type="text/javascript">
						function addDept(){
							location.href="getAllDepartment.action";
						}
					</script>
				</div>
				<div class="span12">
								...
								<tbody>
									<c:forEach var="u" items="${dlist}">
	                                    <tr>
	                                        ...
	                                        <td>${u.maxPeople}</td>
	                                        <td>${u.departmentMark}</td>
	                                        <td>
	                                            <button class="btn btn-info" onclick="allmessage(${u.departmentID})">
	                                                <i class="icon-white icon-eye-open"></i>
	                                               		 查看
	                                            </button>
	                                            <button class="btn btn-warning" onclick="gotomodify(${u.departmentID})">
	                                                <i class="icon-white icon-refresh"></i>
	                                               		 修改
	                                            </button>
	                                            
	                                            <script type="text/javascript">
												  function allmessage(did){
	                                                   location.href="departmentDetails.action?departmentID="+did;
	                                               }
	                                              function gotomodify(did){
	                                                  location.href="initUpdateDepartment.action?departmentID="+did;
	                                              }
	                                            </script>
	                                     	<c:choose>
	                                     		<c:when test="${u.departmentMark == '活动'}">
	                                            <button class="btn btn-danger" onclick="reset(${u.departmentID})">
	                                                <i class="icon-white icon-minus"></i>
	                                                	撤销
	                                            </button>
	                                            </c:when>
	                                            <c:otherwise>
	                                            <button class="btn btn-info"  onclick="reset(${u.departmentID})">
                                                	<i class="icon-white icon-plus"></i>
                                              		 	 恢复
                                            	</button>
                                            	</c:otherwise>
	                                        ...
	                                        <script type="text/javascript">
												function reset(did){
													location.href="resetDepartmentMark.action?departmentID="+did;
												}
											</script> 
	                                        </td>
	                                    </tr>
